...
Page({
  ...
  deleteList: function(e) {
    let that = this;
    var newArray = [];
    //数组中相同的goods_id都删除  单个商品删除
    for (var i = 0; i < that.data.goodsId_arr.length; i++) {
      var count = 0;
      for (var j = 0; j < that.data.goodsId_arr.length; j++) {
        if (that.data.goodsId_arr[i] == that.data.goodsId_arr[j]) {
          count++;
        }
      }
      if (count == 1) {
        newArray.push(that.data.goodsId_arr[i]);
      }
    }
    console.log("去重后的商品id数组：" + newArray);
  },
  Settlement: function(){
